...
**Design philosophy** — rikka-site is resource-oriented, not route-oriented. A URL identifies a *resource* (a noun); HTTP methods are *verbs* on that resource. The site tree is a declarative nesting of `ResourceKind` instances — keys become URL segments (`"users"`, `":userId"`, `":path*"` for catch-all), so there is no routing boilerplate. Each `Kind` is both config and factory: the tree holds the `Kind` (template); per-request, `kind.resolve(params)` creates a `Resource` carrying `params` and `path`. Resources are HTTP-agnostic — they return a `Repr` (`{ content, meta }`), and the server infers transport concerns (status code, `Content-Type`, `Location`, `Allow`) from the Repr's shape, `meta`, and the resource's Kind. Content negotiation is a pluggable transformer pipeline (`Value → Value → Raw → Raw`), not a hardcoded `if (accept === "json")` switch.

**RESTful by construction** — Each Kind constrains the HTTP semantics of its resources: `CollectionKind` → `GET`/`POST`, `ItemKind` → `GET`/`PUT`/`PATCH`/`DELETE`, `SingletonKind` → `GET`/`PUT`/`PATCH`, `ReadOnlyKind` → `GET` only, `ActionKind` → `POST` only. Unsupported methods auto-return `405` with an `Allow` header derived from the methods actually overridden on the resource. One URL serves many representations via `?accept=json|csv|cbor|jsonld|protobuf|html` (explicit) or the `Accept` header with q-values (standard negotiation). Status codes are inferred from the Repr: `201` for `POST` + `meta.location`, `204` for `null` content, `302` for redirect (`Repr.redirect()`), `206` for partial content (`Range` header / `PartialContent`). The `Location` header is auto-set from `meta.location`, resolved relative to the resource path. All IANA-registered HTTP methods — including WebDAV (`PROPFIND`/`MKCOL`/`COPY`/`MOVE`), CalDAV, and DeltaV — are present as named methods on every resource; override the ones you support, the rest return `405`. JSON-LD hypermedia (`@context`/`@id`/`@type`/`@graph`) is produced automatically when `application/ld+json` is requested.

**Prefix-independent** — Resources don't know or care where they're mounted. `meta.location` is expressed relative to the resource's own path (`./42` means "a child of this resource", `../settings` means "a sibling"), and the framework resolves it against the actual request path. The same `ArticleKind` works at `/articles/:id`, `/api/articles/:id`, or `/v2/articles/:id` with zero code changes — the `Location` header, JSON-LD `@id`, and internal links are all derived from the request path. Sub-trees defined once can be mounted at multiple paths without rewriting, which makes resources composable and reusable across sites.

**Automatic content negotiation** — Resource handlers return a structured Value plus a `Schema`, and the framework picks the representation. The handler never inspects `Accept` or branches on output format: the same `list()` method serves `text/html`, `application/json`, `application/ld+json`, `text/csv`, `application/cbor`, and `application/x-protobuf` (when registered) without a single `if`. Negotiation follows `?accept=…` (explicit override) → `Accept` header with q-values (standard) → `application/json` (default for Value content; Raw content uses the resource's `meta.type` or a path-guessed MIME). Browsers sending `Accept: text/html,…` get HTML through negotiation. New formats are added by registering a transformer — no changes to existing resources.
