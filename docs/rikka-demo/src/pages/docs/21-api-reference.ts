...
effect(() => {
  ...
  return () => console.log('cleanup:', value);
        ...
        h3("store<T>(initial)"),
        p(
          "Creates a reactive proxy where each property is a signal. Nested objects are deeply reactive.",
        ),
        pre(
          { class: "code-block" },
          code(
...
user.preferences.theme = 'light';  // reactive`,
        ...
        p(
          {
            style: { color: "#94a3b8", fontSize: "0.875rem", marginTop: "4px" },
          },
          sharedStyles.inlineCode("signalOf()"),
          ": access the underlying ",
          sharedStyles.inlineCode("Signal.State"),
          " for fine-grained reactivity in templates.",
        ),
...
`,
        ...
        table(
          ...
          tbody(
            ...
            tr(
              td(code("Store<T>")),
              td("@rikka/signal"),
              td("Reactive proxy with $-prefix signal access"),
            ),
            tr(
              td(code("StoreSignalRefs<T>")),
              td("@rikka/signal"),
              td("Maps $K → Signal.State<T[K]>"),
            ),
            tr(
              td(code("Child")),
              td("@rikka/dom"),
      ...
      div(
        { class: "doc-nav" },
        a(
          { href: "#/docs/rikka-elements/attach-template", class: "prev-link" },
          "\u2190 attachTemplate",
        ),
        div({ class: "spacer" }),
        a(
          { href: "#/docs/rikka-signal/store", class: "next-link" },
          "store() \u2192",
        ),
      ),
    );
