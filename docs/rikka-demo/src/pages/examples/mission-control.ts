...
const data = store({ cpu: 45, memory: 62, network: 30, disk: 78 });
const peaks = store({ cpu: 45, memory: 62, network: 30, disk: 78 });
...
const ExampleMissionControl = defineElement('rikka-example-mission-control', {
  ...
  render() {
    return div({ class: 'example-page' },
      ...
      p('A dashboard demonstrating realtime updates with store, effect, and computed.'),
      div({ class: 'playground-container' },
        sharedStyles.createPlayground(missionControlCode, '520', 'Mission Control Example'),
      ),
      div({ class: 'explanation' },
        h2('Key Concepts'),
        p(sharedStyles.inlineCode('store({ ... })'), ' creates a reactive object with deep tracking — both values and peaks update automatically.'),
