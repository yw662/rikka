...
const tokenManagerCode = `const colors = store({
  primary: [
...
const groups = computed(() => Object.keys(colors));
...
const loadPreset = () => {
  ...
  if (saved) {
    ...
    Object.keys(data.colors).forEach(key => {
      (colors as any)[key] = data.colors[key];
    });
  }
};
...
const exportCss = () => {
  ...
  Object.keys(colors).forEach(group => {
    (colors as any)[group].forEach((color: {name: string, hex: string}) => {
...
const colorGroup = (groupName: string) => {
  ...
  const groupColors = computed(() => (colors as any)[groupName]);
...
const ExampleTokenManager = defineElement('rikka-example-token-manager', {
  ...
  render() {
    return div({ class: 'example-page' },
      ...
      p('A comprehensive example demonstrating store, effects, and color palette management.'),
      div({ class: 'playground-container' },
        sharedStyles.createPlayground(tokenManagerCode, '600', 'Token Manager Example'),
      ),
      div({ class: 'explanation' },
        h2('Key Concepts'),
        p(sharedStyles.inlineCode('store({ ... })'), ' creates a reactive store for color groups with deep reactivity.'),
