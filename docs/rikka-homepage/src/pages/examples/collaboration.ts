...
const collaborationCode = `const team = signal([
...
const filteredTeam = computed(() => {
  ...
  if (role === 'all') return team.get();
  return team.get().filter(m => m.role === role);
});
...
effect(() => {
  const interval = setInterval(() => {
    ...
    team.set(arr.map((m, i) => i === idx ? {...m, status: newStatus} : m));
